...
const IdeShellLayout: React.FC<IdeShellLayoutProps> = ({ sidebars, children, shellOnly = false }) => {
  ...
  const openWorkspaceTab = React.useCallback((args: OpenWorkspaceTabArgs) => {
    if (args.type === 'catalog') {
      if (metadata?.architectureScope === 'Programme') {
        if (args.catalog !== 'programmes') {
          message.warning('Programme scope: only the Programmes catalog is available.');
          return;
        }
      }

      if (metadata?.architectureScope === 'Domain') {
        const allowed: ReadonlySet<string> = new Set(['applications', 'technologies', 'capabilities']);
        if (!allowed.has(args.catalog)) {
          message.warning(
            'Domain scope: only Application + Technology catalogs are editable. Capabilities are reference-only.',
          );
          return;
        }

        if (args.catalog === 'capabilities') {
          message.info('Domain scope: Capabilities are shown as read-only references.');
        }
      }
  ...
  const sidebarBody = (
    <>
      <div style={{ display: activeSidebarKey === 'explorer' ? 'block' : 'none', height: '100%' }}>
        {explorerBody}
      </div>
      <div style={{ display: activeSidebarKey === 'diagrams' ? 'block' : 'none', height: '100%' }}>
        {diagramsBody}
      </div>
      <div style={{ display: activeSidebarKey === 'analysis' ? 'block' : 'none', height: '100%' }}>
        {analysisBody}
      </div>
      <div style={{ display: activeSidebarKey === 'metamodel' ? 'block' : 'none', height: '100%' }}>
        {metamodelBody}
      </div>
      <div style={{ display: activeSidebarKey === 'settings' ? 'block' : 'none', height: '100%' }}>
        {settingsBody}
      </div>
    </>
  );
        ...
        <Layout className={styles.layoutRoot} style={{ background: token.colorBgLayout }}>
          ...
          <Layout className={styles.mainRow} style={{ background: token.colorBgLayout }}>
            ...
            >
              ...
              <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>{sidebarBody}</div>
            </Layout.Sider>
